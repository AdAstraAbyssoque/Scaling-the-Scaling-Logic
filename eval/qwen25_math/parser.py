...
def extract_answer(pred_str, data_name, use_last_number=True):
    ...
    if data_name in VERBATIM_DATASETS:
        pred = pred_str.strip()
        if "final answer is $" in pred and "$. I hope" in pred:
            tmp = pred.split("final answer is $", 1)[1]
            ...
            pred = pred.split("he answer is")[-1].strip()
        elif "final answer is" in pred:
            pred = pred.split("final answer is")[-1].strip()
        elif "答案是" in pred:
            pred = pred.split("答案是")[1].strip().split("\n\n")[0].strip()
        else:
            code_blocks = re.findall(r"```(?:[\\w+-]+)?\\n([\\s\\S]*?)```", pred)
            if code_blocks:
                pred = code_blocks[-1].strip()
    elif data_name in ["mmlu_stem", "sat_math", "aqua", "gaokao2023"]:
