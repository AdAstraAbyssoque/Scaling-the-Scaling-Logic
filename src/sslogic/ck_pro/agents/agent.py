...
try:
    ...
    def init_phoenix_tracing(*args, **kwargs):
        pass
    ...
    class StepTracer:
        ...
        def add_planning(self, *args, **kwargs):
            pass
        ...
        def add_llm_io(self, *args, **kwargs):
            pass
...
class MultiStepAgent(KwargsInitializable):
    def __init__(self, **kwargs):
        ...
        self.templates = {
            k: get_template(v) for k, v in self.templates.items()
        }  # read real templates from registered ones
        ...
        assert len(ALL_FUNCTIONS) == len(
            self.sub_agents + self.tools
        ), "There may be repeated function names of sub-agents and tools."
    ...
    def __call__(self, task: str, **kwargs):
        ...
        final_results = (
            session.get_current_step().get("end", {}).get("final_results", {})
        )
        ret = AgentResult(
            task=task, session=session, **final_results
        )  # a simple wrapper
        return ret
    ...
    def run(
        self, task, stream=False, session=None, max_steps: int = None, **extra_info
    ):
        start_pc = time.perf_counter()
        rprint(f"ZZStart task for {self.name} [ctime={time.ctime()}]")

        # 使用 context manager 进行任务级别的追踪
        with TaskTracer(task, agent_name=self.name):
            # init session
            if session is None:
            ...
            rprint(
                f"ZZEnd task for {self.name} [ctime={time.ctime()}, interval={time.perf_counter()-start_pc}]"
            )
            return ret
    ...
    def yield_session_run(self, session, max_steps):
        ...
        while True:
            ...
            _error_counts = sum(
                self.get_obs_str(z["action"]).strip().startswith(CODE_ERROR_PERFIX)
                for z in session.steps
            )
            if (step_idx >= max_steps + _error_counts) or (
                step_idx >= int(max_steps * 1.5)
            ):  # make up for the errors (but avoid too many steps)
                stop_reason = StopReasons.MAX_STEP  # step limit
                break
            if (self.max_time_limit > 0) and (
                (time.perf_counter() - start_pc) > self.max_time_limit
            ):
    ...
    def step(self, session, state):
        ...
        step_idx = _current_step.get("step_idx", 0)

        # 使用 context manager 进行步骤级别的追踪
        with StepTracer(step_idx, step_type="react") as step_tracer:
            # planning
            ...
            if has_plan_template:  # planning to update state
                ...
                # 记录规划阶段的 LLM I/O
                step_tracer.add_llm_io("planning", plan_messages, plan_response)
                step_tracer.add_planning(plan_res.get("thought", ""), state)
                ...
                if plan_res["code"]:
                    try:
                        ...
                        zwarn(f"Failed to eval plan code: {e}")
                        zwarn(
                            f"Code snippet (first 500 chars): {plan_res['code'][:500]}"
                        )
                        new_state = None
                    if new_state:  # note: inplace update!
                        ...
                        zwarn("State NOT changed due to empty output or eval failure!")
                else:
                    # if jailbreak detected, change the experience state by fource.
                    if (
                        plan_res["thought"]
                        == "Jailbreak or content filter violation detected. Please modify your prompt or stop with N/A."
                    ):
                        if "experience" in state:
                            state["experience"].append(
                                f'Jailbreak or content filter violation detected for the action {_input_kwargs["recent_steps_str"].split("Action:")[1]}. Please modify your prompt or stop with N/A.'
                            )
                        else:
                            state["experience"] = []
                ...
                plan_res["state"] = (
                    state.copy()
                )  # after updating the progress state (make a copy)
                if self.store_io:  # further storage
                    plan_res.update(
                        {"llm_input": plan_messages, "llm_output": plan_response}
                    )
                yield {"type": "plan", "step_info": _current_step}

            # predict action
            _action_input_kwargs = _input_kwargs.copy()
            _action_input_kwargs["state"] = json.dumps(
                state, ensure_ascii=False, indent=2
            )  # there can be state updates
            ...
            # 记录动作阶段的 LLM I/O 和 Thought/Action
            step_tracer.add_llm_io("action", action_messages, action_response)
            step_tracer.add_action(
                action_res.get("thought", ""), action_res.get("code", "")
            )

            # perform action
            step_res = self.step_action(
                action_res, _action_input_kwargs, **_extra_kwargs
            )

            # 记录 Observation
            step_tracer.add_observation(step_res)
            ...
            if self.store_io:  # further storage
                action_res.update(
                    {"llm_input": action_messages, "llm_output": action_response}
                )
    ...
    def finalize(self, session, state, stop_reason: str):
        ...
        if (
            has_end_template
        ):  # we have an ending module to further specify final results
            _input_kwargs, _extra_kwargs = self.step_prepare(session, state)
            # --
            # special ask_llm if not normal ending
            if stop_reason != StopReasons.NORMAL_END and hasattr(self, "tool_ask_llm") and self.tool_ask_llm is not None:
        ...
        _current_step = session.get_current_step()
        if (
            has_end_template or final_results is None
        ):  # try to get final results, end_module can override final_results
            try:
                final_results = eval(end_res["code"])
                assert (
                    isinstance(final_results, dict)
                    and "output" in final_results
                    and "log" in final_results
                )
            except Exception as e:  # use the final step's observation as the result!
                _log = (
                    "We are returning the final step's answer since there are some problems in the finalizing step."
                    if has_end_template
                    else ""
                )
    ...
    def get_obs_str(self, action, obs=None, add_seq_enum=True, truncate_error=False):
        """
        获取观察结果的字符串表示
        ...
        Args:
            ...
            obs: 观察结果（可选）
            add_seq_enum: 是否添加序号
            truncate_error: 是否截断错误信息（用于历史步骤）
        """
        if obs is None:
        ...
        def _safe_str(value):
            text = str(value)
            if len(text) > self.obs_max_token:
                text = (
                    text[: self.obs_max_token]
                    + f" ... (截断，超过 {self.obs_max_token} 字符)"
                )
            return text

        def _smart_truncate_error(error_text):
            """
            智能截断错误信息：保留错误类型和关键信息，截断详细堆栈
            """
            if len(error_text) <= self.obs_max_token:
                return error_text

            lines = error_text.split("\n")
            # 提取错误摘要（前3行通常包含错误类型和位置）
            summary_lines = []
            detail_lines = []
            ...
            for i, line in enumerate(lines):
                if i < 3 or any(
                    keyword in line
                    for keyword in ["Error:", "Exception:", "Traceback", 'File "']
                ):
            ...
            # 构建截断后的错误信息
            summary = "\n".join(summary_lines)
            if len(summary) > self.obs_max_token * 0.7:  # 如果摘要本身就很长
                summary = summary[: int(self.obs_max_token * 0.7)]

            result = summary + f"\n... (详细堆栈已截断，共 {len(lines)} 行)"
            return result
        ...
        if isinstance(obs, (list, tuple)):
            for idx, item in enumerate(obs):
                processed.append(
                    (
                        f"- Result {idx}: {_safe_str(item)}"
                        if add_seq_enum
                        else _safe_str(item)
                    )
                )
        elif isinstance(obs, str) and obs.startswith("Code Execution Error:"):
            if truncate_error:
                # 历史步骤：智能截断，保留关键信息
                processed.append(_smart_truncate_error(obs))
            else:
                # 当前步骤：保留完整报错
                processed.append(obs)
        else:
            processed.append(_safe_str(obs))
    ...
    def _prepare_common_input_kwargs(self, session, state):
        # previous steps
        _recent_steps = session.get_latest_steps(
            count=self.recent_steps
        )  # no including the last which is simply empty
        _recent_steps_str = "\n\n".join(
            [
                f"### Step {ss['step_idx']}\nThought: {ss['action']['thought']}\nAction: ```\n{ss['action']['code']}```\nObservation: {self.get_obs_str(ss['action'], truncate_error=True)}"
                for ii, ss in enumerate(_recent_steps)
            ]
        )
        _current_step = session.get_current_step()
        _current_step_action = _current_step.get("action", {})
        _current_step_str = f"Thought: {_current_step_action.get('thought')}\nAction: ```\n{_current_step_action.get('code')}```\nObservation: {self.get_obs_str(_current_step_action, truncate_error=False)}"
        # tools and sub-agents
        ret = {
            "task": session.task,
            "state": json.dumps(state, ensure_ascii=False, indent=2),
            "recent_steps": _recent_steps,
            "recent_steps_str": _recent_steps_str,
            "current_step": _current_step,
            "current_step_str": _current_step_str,
        }
        for short in [True, False]:
            _subagent_str = "## Sub-Agent Functions\n" + "\n".join(
                [z.get_function_definition(short) for z in self.sub_agents]
            )
            _tool_str = "## Tool Functions\n" + "\n".join(
                [z.get_function_definition(short) for z in self.tools]
            )
    ...
    def step_action(self, action_res, action_input_kwargs, **kwargs):
        ...
        python_executor.add_global_vars(
            **self.ACTIVE_FUNCTIONS
        )  # to avoid that things might get re-defined at some place ...
        _exec_timeout = (
            self.exec_timeout_with_call
            if any((z in action_res["code"]) for z in self.sub_agent_names)
            else self.exec_timeout_wo_call
        )  # choose timeout value
        python_executor.run(
            action_res["code"], catch_exception=True, timeout=_exec_timeout
        )  # handle err inside!
        ret = (
            python_executor.get_print_results()
        )  # currently return a list of printed results
