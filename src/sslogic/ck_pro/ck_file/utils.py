...
class FileEnv(KwargsInitializable):
    def __init__(self, starting=True, starting_file_path_dict=None, **kwargs):
        ...
        self.workspace_root = os.path.abspath(kwargs.pop("workspace_root", os.getcwd()))
    ...
    def _resolve_workspace_path(self, target: str = ".", *, allow_missing: bool = True) -> Path:
        candidate = Path(target or ".")
        if not candidate.is_absolute():
            candidate = (self._workspace_root_path / candidate).resolve()
        else:
            candidate = candidate.resolve()
        try:
            candidate.relative_to(self._workspace_root_path)
        except ValueError:
            if candidate != self._workspace_root_path:
                raise PermissionError(
                    f"Access to path '{candidate}' is outside of workspace root '{self._workspace_root_path}'."
                )
    ...
    def _pretty_path(self, path: Path) -> str:
        try:
            return str(path.relative_to(self._workspace_root_path))
        except ValueError:
            return str(path)
                ...
                if m:
                    ...
                    if key in ["read_text", "read_screenshot"]:
                        ...
                        if not target or target == "()":
                            target = "."
                        action["target_file"] = target.strip().strip('"').strip("'")
                    elif key == "cat":
                        ...
                        max_bytes = parts[1] if len(parts) > 1 else ""
                        action["target_file"] = file_name.strip('"').strip("'")
                        action["max_bytes"] = max_bytes.strip()
                    elif key == "bash":
    ...
    def action(self, action):
        ...
        elif action["action_name"] == "ls":
            try:
                target_path = self._resolve_workspace_path(action["target_file"] or ".")
                entries = sorted(os.listdir(target_path))
                textual_content = "\n".join(entries) if entries else "(empty)"
                observation = f"ls({self._pretty_path(target_path)})  # {len(entries)} entries"
                file_name = self._pretty_path(target_path)
            except Exception as e:
                error_message = f"{e}"
                textual_content = ""
                observation = f"ls({action['target_file'] or '.'})  # error"
        elif action["action_name"] == "cat":
            try:
                target_path = self._resolve_workspace_path(action["target_file"], allow_missing=False)
                max_bytes = action.get("max_bytes")
                try:
                    max_bytes_int = int(max_bytes) if max_bytes not in (None, "") else 8000
                except ValueError:
                    max_bytes_int = 8000
                with target_path.open("rb") as handle:
                    data = handle.read(max_bytes_int + 1)
                truncated = len(data) > max_bytes_int
                if truncated:
                    data = data[:max_bytes_int]
                try:
                    textual_content = data.decode("utf-8", errors="replace")
                except Exception:
                    textual_content = data.decode("utf-8", errors="replace")
                if truncated:
                    textual_content += "\n... (truncated)"
                    error_message = f"Content truncated to first {max_bytes_int} bytes."
                observation = f"cat({self._pretty_path(target_path)}, max_bytes={max_bytes_int})"
                file_name = self._pretty_path(target_path)
            except Exception as e:
                error_message = f"{e}"
                textual_content = ""
                observation = f"cat({action['target_file']})  # error"
        elif action["action_name"] == "bash":
            command = action.get("command") or ""
            try:
                if not command:
                    raise ValueError("Command string is empty.")
                tokens = shlex.split(command, posix=True)
                destructive_verbs = {"rm", "sudo", "mv", "chmod", "chown", "rmdir", "mkfs", "dd", "kill", "reboot", "shutdown"}
                if tokens and tokens[0] in destructive_verbs:
                    raise PermissionError("Destructive or unsupported command detected; aborting execution.")
                forbidden_patterns = ["; rm", "&& rm", "|| rm", ">/", ">>", ":>", "truncate", "chmod", "chown"]
                ...
                textual_pieces = []
                if stdout:
                    textual_pieces.append(stdout)
                if stderr:
                    textual_pieces.append(f"[stderr]\n{stderr}")
                textual_content = "\n".join(textual_pieces) if textual_pieces else "(no output)"
                if result.returncode != 0 and not stderr:
                    error_message = f"Command exited with code {result.returncode}."
                elif result.returncode != 0:
                    error_message = f"Command exited with code {result.returncode}. Stderr captured."
                observation = f"bash({command})"
                file_name = "<bash>"
            except Exception as e:
                error_message = f"{e}"
                textual_content = ""
                observation = f"bash({command})  # error"
