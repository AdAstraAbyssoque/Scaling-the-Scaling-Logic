...
class FileAgent(MultiStepAgent):
    def __init__(self, **kwargs):
        ...
        self.ACTIVE_FUNCTIONS.update(
            stop=self._my_stop,
            load_file=self._my_load_file,
            read_text=self._my_read_text,
            read_screenshot=self._my_read_screenshot,
            search=self._my_search,
            ls=self._my_ls,
            cat=self._my_cat,
            bash=self._my_bash,
        )
    ...
    def _my_ls(self, path: str = "."):
        return ActionResult(f"ls({path})")

    def _my_cat(self, file_path: str, max_bytes: int = 8000):
        return ActionResult(f"cat({file_path}, {max_bytes})")

    def _my_bash(self, command: str):
        return ActionResult(f"bash({command})")
