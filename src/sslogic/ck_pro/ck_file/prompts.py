...
- ls(path: str = ".") -> str:  # list files and folders under `path` (relative to the workspace root, defaults to current directory).
- cat(file_name: str, max_bytes: int = 8000) -> str:  # read text content from `file_name` (relative to workspace) with an optional byte limit; returns UTF-8 text.
- bash(command: str) -> str:  # execute a safe read-only shell command (e.g., `ls`, `cat`, `head`) within the workspace and return stdout/stderr.
...
6. **Use Shell Helpers Sparingly**: Prefer `ls`, `cat`, and `bash` for light-weight workspace inspection. Keep commands read-only; destructive commands are blocked.
7. **Stop with Unrecoverable Errors**: If you encounter unrecoverable errors or cannot complete the target tasks after several tryings, issue the `stop` action with an empty response and provide detailed reasons for the failure.
8. **Use Defined Functions**: Strictly use defined functions for file operations, load_file() for loading data, read_text() for text extraction, read_screenshot() for image processing. Do not use alternative Python libraries or custom file handling methods.
9. **Load before Read**: Please use load_file to load the files first and then read_text or read_screenshot.
10. **Use python code to load if load_file failed**: If load_file failed, try to load the file using python code. For example, when encoutering a zip file, try to unzip it first and then load other files.
11. **Use search only on very long documents and when identifying the exact keywords is needed**: Shorter documents, or the cases when exact match is not required, can be readed by read_text or read_screenshot directly.
...
def _prepare_imgs(image_suffix, visual_content):
    ...
    if len(image_suffix) < len(visual_content):
        image_suffix = image_suffix + ["png"] * (
            len(visual_content) - len(image_suffix)
        )
    ret = [
        {"type": "image_url", "image_url": {"url": f"data:image/{s};base64,{img}"}}
        for s, img in zip(image_suffix, visual_content)
    ]
    return ret
...
def file_plan(**kwargs):
    ...
    user_lines.append(
        f"## Paths of all Files\n{kwargs['loaded_files']}\n\n"
    )  # file information
    user_lines.append(
        f"## Meta information of all Files\n{kwargs['file_meta_data']}\n\n"
...
"""
    ...
    ret = [
        {"role": "system", "content": _FILE_PLAN_SYS},
    ...
    if kwargs["visual_content"] is not None:
        ...
        ret[-1]["content"] = [
            {
                "type": "text",
                "text": ret[-1]["content"] + "\n\n## Screenshot of the current pages.",
            }
        ] + _prepare_imgs(kwargs["image_suffix"], kwargs["visual_content"])
...
"""
    ...
    ret = [
        {
            "role": "system",
            "content": _FILE_ACTION_SYS.replace(
                "MAX_FILE_READ_TOKENS", str(kwargs["max_file_read_tokens"])
            ).replace("MAX_FILE_SCREENSHOT", str(kwargs["max_file_screenshots"])),
        },
        {"role": "user", "content": user_str},
    ]
    if kwargs["visual_content"] is not None:
...
"""
    ...
    user_str = "".join(user_lines)
    ret = [
        {"role": "system", "content": _FILE_END_SYS},
        {"role": "user", "content": user_str},
    ]
    return ret
