...
default_file_configs = {
    "model": {"call_target": "api:gpt-4o"},  # LLM target (configurable via .env)
    ...
    if input_file:
        with open(input_file) as fd:
            for line in fd:
                if line.strip():
                    ...
                    file_path_dict = get_until_hit(
                        info_field,
                        ["file_path_dict", "file_name"],
                        df=DEFAULT_START_PAGE,
                    )
                    ...
                    if task:
                        if isinstance(file_path_dict, str):
                            ...
                            file_path_dict = {
                                os.path.join(file_base_dir, file): ""
        ...
        while True:
            ...
            if _special_prefix in task:
                ...
                task, file_path_dict = (
                    task[:_idx],
                    task[_idx + len(_special_prefix) - 4 :],
                )
            else:
...
def main():
    ...
    init_phoenix_tracing(project_name="ck-pro-file")
    ...
    with my_open_with(args.output, "w") as fout:
        for inst in yield_inputs(args.input):
            res_session = file_agent.run(
                inst["task"], file_path_dict=inst["file_path_dict"]
            )
