...
_WEB_PLAN_SYS = (
    """You are an expert task planner, responsible for creating and monitoring plans to solve web agent tasks efficiently.
...
_WEB_ACTION_SYS = (
    """You are an intelligent assistant designed to navigate and interact with web pages to accomplish specific tasks. Your goal is to generate Python code snippets using predefined action functions.
...
**CRITICAL: The Code section must contain ONLY a valid Python dictionary that can be directly evaluated by Python's eval() function. Follow these strict rules:**
1. Use double quotes (") for all dictionary keys and string values
2. If a string value contains double quotes, escape them with backslash: \\"
3. Do NOT include any Python code, function calls, or print statements
4. Do NOT add any comments or explanations inside the code block
5. Do NOT nest code blocks or include Action/Next steps within the dictionary
6. The entire code block must be a single, valid Python dictionary literal

**Example of CORRECT format:**
```python
{
    "completed_list": ["Step 1", "Step 2"],
    "todo_list": ["Search for information about topic"],
    "experience": [],
    "downloaded_files": {},
    "information": ["The article mentions that results show 524,000 entries"]
}
```
...
```python
{
    "todo_list": ["Search for 'Lights Out puzzle'"]  # WRONG: nested single quotes
}
```
OR
```python
{
    "completed_list": ["Step 1"]
}
print(click(id=5))  # WRONG: code after dictionary
```
"""
    # --
    ...
    ret = [
        {"role": "system", "content": _WEB_PLAN_SYS},
...
def web_action(**kwargs):
    ...
    if kwargs.get("html_md"):  # text representation
        user_content[-1][
            "text"
        ] += f"## Markdown Representation of Current Page\n{kwargs['html_md']}\n\n"
    user_content[-1][
        "text"
    ] += f"## Current Accessibility Tree\n{kwargs['web_page']}\n\n"
    if kwargs.get("screenshot"):
...
**CRITICAL: The Code section must contain ONLY executable Python code using the predefined action functions. Follow these strict rules:**
1. Issue ONLY ONE action per code block (e.g., one click(), one type(), one scroll_down(), etc.)
2. Always use print() to output the result of your action
3. Do NOT mix action code with dictionary definitions
4. Do NOT include multiple actions in one code block
5. Use proper string escaping if your content contains quotes

**Example of CORRECT format:**
```python
print(type(id=5, content="latest iphone information"))
```
OR
```python
result = scroll_down()
...
print(click(id=5))  # WRONG: mixing dictionary and action
```
"""
    if len(user_content) == 1 and user_content[0]["type"] == "text":
        user_content = user_content[0]["text"]  # directly use the str!
    ret = [
        {"role": "system", "content": _WEB_ACTION_SYS},
        {"role": "user", "content": user_content},
    ]  # still use the old format
    # if kwargs.get('screenshot'):
    #     ret[-1]['content'] = [
    #         {'type': 'text', 'text': ret[-1]['content'] + "\n\n## Screenshot of the current webpage."},
    #         {'type': 'image_url', 'image_url': {"url": f"data:image/png;base64,{kwargs['screenshot']}"}},
...
**CRITICAL: The Code section must contain ONLY a valid Python dictionary with 'output' and 'log' fields. Follow these strict rules:**
1. Use double quotes (") for all dictionary keys and string values
2. If a string value contains double quotes, escape them with backslash: \\"
3. Do NOT include any Python code or function calls
4. The dictionary must have exactly two keys: "output" and "log"
5. Both values should be strings

**Example of CORRECT format:**
```python
{
    "output": "The price of the iPhone 16 is $799.",
    "log": "Task completed successfully. Navigated to Apple website, found pricing page, extracted price information."
}
```
"""
    )
    ...
    ret = [
        {"role": "system", "content": _WEB_END_SYS},
