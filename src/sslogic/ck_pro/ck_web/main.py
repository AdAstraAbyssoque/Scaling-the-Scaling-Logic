...
from ..agents.phoenix_tracer import init_phoenix_tracing

default_web_configs = {
    "model": {"call_target": "api:gpt-4o"},  # LLM target (configurable in .env)
    # "web_env_kwargs": {"web_ip": "localhost:8833"},  # IP for the web-browser server
}
        ...
        with open(input_file) as fd:
            for line in fd:
                if line.strip():
                    ...
                    target_url = get_until_hit(
                        info_field, ["target_url"], df=DEFAULT_START_PAGE
                    )
                    if task:
                        yield {
                            "id": f"task{_idx:04d}",
...
def kill_web():
    ...
    try:
        ...
        result = subprocess.run(
            ["pgrep", "-f", "run_local_mac.sh"], capture_output=True, text=True
        )
        ...
        port_result = subprocess.run(
            ["lsof", "-t", "-i:3000"], capture_output=True, text=True
        )
...
def start_web():
    ...
    try:
        ...
        script_directory = Path(__file__).resolve().parent / "_web"
        subprocess.Popen(["sh", "run_local_mac.sh"], cwd=str(script_directory))
...
def main():
    ...
    init_phoenix_tracing(project_name="ck-pro-web")
