...
def web_click(id: int, link_name=""):
    return ActionResult(f"click [{id}] {link_name}")


def web_type(id: int, content: str, enter=True):
    return ActionResult(
        f"type [{id}] {content}" if enter else f"type [{id}] {content}[NOENTER]"
    )


def web_scroll_up():
    return ActionResult(f"scroll up")


def web_scroll_down():
    return ActionResult(f"scroll down")


def web_wait():
    return ActionResult(f"wait")


def web_goback():
    return ActionResult(f"goback")


def web_restart():
    return ActionResult(f"restart")


def web_goto(url: str):
    return ActionResult(f"goto {url}")
...
class WebAgent(MultiStepAgent):
    def __init__(self, **kwargs):
        ...
        self.use_multimodal = (
            "auto"  # no: always no, yes: always yes, auto: let the agent decide
        )
        ...
        self.ACTIVE_FUNCTIONS.update(
            click=web_click,
            type=web_type,
            scroll_up=web_scroll_up,
            scroll_down=web_scroll_down,
            wait=web_wait,
            goback=web_goback,
            restart=web_restart,
            goto=web_goto,
        ...
        self.ACTIVE_FUNCTIONS.update(
            stop=self._my_stop, save=self._my_save, screenshot=self._my_screenshot
        )
        # --
    ...
    def _my_save(self, remote_path: str, local_path: str):
        try:
            ...
            if local_path != remote_path:
                ...
                if remote_path.startswith("http://") or remote_path.startswith(
                    "https://"
...
```"""
    ...
    def step_call(self, messages, session, model=None):
        _use_multimodal = session.info.get(
            "use_multimodal", False
        ) or have_images_in_messages(messages)
        if model is None:
            model = (
                self.model_multimodal if _use_multimodal else self.model
            )  # use which model?
    ...
    def step_prepare(self, session, state):
        ...
        if session.num_of_steps() > 1:  # has previous step
            ...
            _input_kwargs.update(
                self._prep_page(_prev_step["action"]["web_state_before"], suffix="_old")
            )
        else:
        ...
        if (
            session.num_of_steps() >= self.check_nodiff_steps
            and self.check_nodiff_steps > 1
        ):
            _check_pages = [
                self._prep_page(z["action"]["web_state_before"])
                for z in session.get_latest_steps(count=self.check_nodiff_steps - 1)
            ] + [_this_page_info]
            if all(z == _check_pages[0] for z in _check_pages):  # error
                _input_kwargs["web_page"] = (
                    _input_kwargs["web_page"]
    ...
    def step_action(self, action_res, action_input_kwargs, web_env=None, **kwargs):
        action_res["web_state_before"] = (
            web_env.get_state()
        )  # inplace storage of the web-state before the action
        _rr = super().step_action(
            action_res, action_input_kwargs
        )  # get action from code execution
        ...
        try:  # execute the action on the browser
            ...
            ret = (
                action_result if action_result is not None else step_result
            )  # use action result if there are direct ones
            web_env.sync_files()
    ...
    def _prep_page(self, web_state, suffix=""):
        ...
        if self.use_multimodal == "on":  # always on
            ...
            ret["screenshot_note"] = (
                "The current system does not support webpage screenshots. Please refer to the accessibility tree to understand the current webpage."
            )
            ...
            if web_state.get("curr_screenshot_mode"):  # currently on
                ...
                ret["screenshot_note"] = (
                    "The current system's screenshot mode is off. If you need the screenshots, please use the corresponding action to turn it on."
                )
        # --
        if suffix:
    ...
    def _prep_html_md(self, web_state):
        ...
        if _budget <= 0:
            return ""
        # --
        axtree, html_md = web_state["current_accessibility_tree"], web_state.get(
            "html_md", ""
        )
                ...
                if one_t in html_lines[_curr]:  # hit
                    hit_lines.update(
                        [
                            ii
                            for ii in range(
                                _curr - _LOCAL_WINDOW, _curr + _LOCAL_WINDOW + 1
                            )
                            if ii >= 0 and ii < len(html_lines)
                        ]
                    )  # add local window
                    _last_hit = _curr
                    break
    ...
    def get_call_stat(self, clear: bool):
        """
        重写父类方法以包含 multimodal 模型的统计

        返回格式：
        {
            "__main__": {...},           # 主模型统计
            "__multimodal__": {...},     # 多模态模型统计
            "sub_agent_name": {...},     # 子 agent 统计
            "__ALL__": {...}             # 总计
        }
        """
        from collections import Counter

        # 获取父类的统计（包括主模型和子 agent）
        ret = super().get_call_stat(clear=clear)

        # 添加 multimodal 模型的统计
        multimodal_stat = self.model_multimodal.get_call_stat(clear=clear)
        if multimodal_stat:  # 只有在有统计数据时才添加
            ret["__multimodal__"] = multimodal_stat

            # 更新总计：将 multimodal 的统计累加到 __ALL__
            if "__ALL__" in ret:
                ret["__ALL__"].update(multimodal_stat)
            else:
                ret["__ALL__"] = Counter(multimodal_stat)
