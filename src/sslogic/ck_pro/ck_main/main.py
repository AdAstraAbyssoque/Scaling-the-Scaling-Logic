...
from ..agents.phoenix_tracer import init_phoenix_tracing, is_phoenix_enabled
...
default_main_configs = {
    "model": {
        "call_target": "mock"
    },  # LLM target (override via CLI or config)
    "web_agent": {
        "model": {
            "call_target": "mock"
        },  # LLM target for the web agent (override via CLI or config)
        "web_env_kwargs": {
            "web_ip": "localhost:8833"
        },  # IP for the web-browser server (Docker container)
    },
}
...
def get_args():
    ...
    parser.add_argument(
        "-u", "--updates", type=str, default=[], nargs="+"
    )  # updating dicts
    parser.add_argument("-i", "--input", type=str, default="")
    parser.add_argument("-o", "--output", type=str, default="")
    parser.add_argument(
        "-S", "--sep_run_root", type=str, default=""
    )  # separate running root dir for each task, empty if not enabled
    parser.add_argument(
        "-P", "--try_preload_output", type=int, default=1
    )  # try loading the already processed outputs
    parser.add_argument(
        "--starting_idx", type=int, default=0
    ...
    parser.add_argument(
        "--sampling-mode", action="store_true"
    )  # in sampling (trajectory) model, will use an evaluator to check whether the query has finished (with an answer)
    parser.add_argument(
        "--evaluation-method",
        choices=["disabled", "em", "llm_score", "stop_with_answer"],
        default="disabled",
    )  # useful when --sampling-mode is on.
    ...
    parser.add_argument(
        "--inference-time-evaluation-method",
        choices=[
            "disabled",
            "no_answer",
            "no_answer+no_ask_llm",
            "gpt_judge",
            "ensemble",
            "gpt_judge+ensemble",
        ],
        default="disabled",
    )  # whether to enable an auto evaluator and perform reflection
    parser.add_argument(
        "--max_retry_num", default=3, type=int
    )  # maximum number of retries when sampling-mode or inference_time_evaluation_method is on.
    parser.add_argument("--reflection", type=bool, default=False)
    parser.add_argument(
        "--save_failed_tries", action="store_true"
    )  # whether to save "failed" tries. Can disable this when running inference on test set.
        ...
        with open(input_file) as fd:
            for line in fd:
                if line.strip():
                    ...
                    answer = get_until_hit(
                        info_field, ["Final answer", "answer", "true_answer"]
                    )
                    if (
                        get_until_hit(info_field, ["skip"]) is None
                        or str(get_until_hit(info_field, ["skip"])) != "1"
                    ):
                        skip_hard = False
                    else:
                        skip_hard = True
...
def main():
    ...
    # 初始化 Phoenix tracing（如果启用）
    # 设置 PHOENIX_ENABLE=true 环境变量以启用
    phoenix_enabled = init_phoenix_tracing(project_name="ck-pro-main")
    if phoenix_enabled:
        rprint("✓ Phoenix tracing 已启用，访问 http://localhost:6006 查看追踪")
    ...
    input_dir = os.getenv(
        "FILE_BASE_DIR", default=os.path.dirname(os.path.abspath(args.input))
    )
    ...
    with my_open_with(args.output, "w") as fout:
        for inst in yield_inputs(args.input):
            ...
            if args.sep_run_root:
                ...
                if inst.get("file"):
                    _input_file = (
                        "input." + inst["file"].split(".")[-1]
                    )  # make a simpler name!
                    shutil.copy(
                        os.path.join(input_dir, inst["file"]),
                        os.path.join(trg_dir, _input_file),
                    )
                os.chdir(trg_dir)  # switch to specific working dir
            else:
                _input_file = (
                    os.path.join(input_dir, inst["file"]) if inst.get("file") else None
                )
            ...
            if inst["id"] in existing_inst_map:  # simply load it
                ...
                if exist_inst["task"] != inst["task"]:
                    zwarn(
                        f"Ignore mismatched instances: {exist_inst['task']} vs {inst['task']}"
                    )
                else:
                    rprint(
                        f"Directly load the previous run session without running for {inst['id']}"
                    )
            ...
            if "session" not in inst:
                    ...
                    if args.sampling_mode:
                        ...
                        if args.evaluation_method == "disabled":
                            ...
                            while _try_num < args.max_retry_num:
                                _try_num += 1
                                if ck_evaluator.evaluate_with_answer(
                                    res_session.to_dict(),
                                    str(inst.get("answer", "UNK")),
                                    inst["task"].strip(),
                        ...
                        if args.inference_time_evaluation_method == "disabled":
                            ...
                            for i in range(candidate_num):
                                rprint(
                                    f"Start to run task {inst['id']} for the {i+1} time",
                                    timed=True,
                                )
                                feedback = None
                                feedback_list = []
                                for j in range(args.max_retry_num):
                                    if args.reflection:
                                        new_task = (
                                            f"{_task}. Here is a feedback for a previous try that failed:\n\n{feedback}"
                                            if feedback
                                            else _task
                                        )
                                    else:
                                    ...
                                    res_session_list.append(res_session)
                                    has_failure, feedback = ck_evaluator.detect_failure(
                                        res_session.to_dict(),
                                        evaluation_type=args.inference_time_evaluation_method,
                                    )
                                    if not has_failure:
                                        break
                                    print(
                                        f"Retrying task {inst['id']} due to {feedback}"
                                    )
                                    feedback_list.append(feedback)
                                candidate_sessions.append(res_session)
                            if "ensemble" in args.inference_time_evaluation_method:
                                res_session = candidate_sessions[
                                    ck_evaluator.ensemble(
                                        [x.to_dict() for x in candidate_sessions]
                                    )
                                ]
                ...
                if res_session is None:  # error?
                        ...
                        "steps": [
                            {
                                "step_idx": -1,
                                "end": {
                                    "final_results": {"output": "error", "log": "error"}
                                },
                            }
                        ]
                    }
                    ...
                    if args.save_failed_tries and len(res_session_list) > 1:
                        inst["previous_failed_sessions"] = [
                            sess.to_dict() for sess in res_session_list[:-1]
                        ]
            # --
            # simple EVAL
            ...
            try:
                answer_pred = str(
                    inst["session"]["steps"][-1]["end"]["final_results"]["output"]
                )
            except:
                answer_pred = "error"
            total_task += 1
            _this_corr = int(
                question_scorer(model_answer=answer_pred, ground_truth=answer_gold)
            )
            ...
            rprint(
                f"Current Processing Accuracy = {corr_task}/{total_task}={corr_task/total_task:.4f}"
            )
            # =====
            # save
            if args.sep_run_root:
            ...
            if fout:
                try:
                    fout.write(
                        my_json_dumps(tuple_keys_to_str(inst), ensure_ascii=False)
                        + "\n"
                    )
                except:
