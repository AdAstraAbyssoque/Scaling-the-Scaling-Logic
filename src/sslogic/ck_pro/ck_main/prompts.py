...
def _trim_text(text: str, max_chars: int = 600) -> str:
    if text is None:
        return ""
    text = text.strip()
...
def _limit_list(items, max_items: Optional[int] = 7, item_chars: int = 300):
    if not isinstance(items, list):
        return items
    if max_items is None:
        return [_trim_text(str(it), item_chars) for it in items]
    if max_items <= 0:
        return []
    kept = items[-max_items:]
    processed = [_trim_text(str(it), item_chars) for it in kept]
    if len(items) > max_items:
        processed.insert(0, f"... ({len(items) - max_items} earlier entries omitted)")
    return processed


def _summarize_state(state_str: str, max_items: int = 7, item_chars: int = 300) -> str:
    try:
        state = json.loads(state_str)
    except Exception:
        return _trim_text(state_str, max_chars=1200)
    summarized = {}
    unfolded_keys = {
        "completed_list",
        "todo_list",
    ...
    for key, value in state.items():
        if isinstance(value, list):
            per_key_max_items = None if key in unfolded_keys else max_items
            summarized[key] = _limit_list(
                value, max_items=per_key_max_items, item_chars=item_chars
            )
        elif isinstance(value, dict):
            summarized[key] = {
                k: _trim_text(str(v), item_chars)
                for k, v in list(value.items())[:max_items]
            }
            if len(value) > max_items:
                summarized[key][
                    "__note__"
                ] = f"... ({len(value) - max_items} additional keys omitted)"
        else:
            summarized[key] = _trim_text(str(value), item_chars)
    return json.dumps(summarized, ensure_ascii=False, indent=2)


def _summarize_recent_steps(
    steps_str: str, max_steps: int = 5, per_step_chars: int = 4000
) -> str:
    if not steps_str:
        return "(无最近步骤)"
    blocks = [blk.strip() for blk in steps_str.strip().split("\n\n") if blk.strip()]
    total = len(blocks)
    if total == 0:
        return "(无最近步骤)"
    if total > max_steps:
        blocks = blocks[-max_steps:]
        prefix = f"(Earlier {total - max_steps} steps omitted)\n"
    else:
        prefix = ""
    trimmed = [_trim_text(block, per_step_chars) for block in blocks]
    return prefix + "\n\n".join(trimmed)


def _summarize_current_step(step_str: str, max_chars: int = 4000) -> str:
    return _trim_text(step_str, max_chars=max_chars)
...
4. **Multi-agent Collaboration Patterns**:
    ...
    - **Typical Workflows**:
        ...
        - Example 3, Reading Simple Text Files: (1) For simple text files (e.g., .txt, .json, .md, .yaml), use `read_text` directly to get the content quickly. (2) For complex file types (PDF, Excel, images) or when advanced processing is needed, use `file_agent`.
...
_CK_PLAN_SYS = (
    """You are a strategic assistant responsible for the high-level planning module of the Cognitive Kernel, an initial autopilot system designed to accomplish user tasks efficiently.
...
2. **Code Generation**: Create a Python dictionary representing the updated state. Ensure it is directly evaluable using the eval function. Check the `Progress State` section above for the required content and format for this dictionary. Keep every string value on a single line—do not embed raw newline characters inside string literals.
...
## Output Format
**CRITICAL: You MUST output EXACTLY ONE response with ONE Thought and ONE Code block. DO NOT output multiple Thought-Code pairs. DO NOT continue thinking after the Code block.**

Your response should strictly follow this format:
Thought: {Provide your reasoning and explanation}
Code: {Output your Python dictionary wrapped in ```python ``` marks}

**After outputting the Code block, STOP IMMEDIATELY. Do NOT add any additional Thought, Observation, or Code blocks.**
"""
    + _CK_STRATEGY
)

_CK_ACTION_SYS = (
    """You are a strategic assistant responsible for the action module of the Cognitive Kernel, an initial autopilot system designed to accomplish user tasks. Your role is to generate a Python code snippet to execute the next action effectively.
...
Your response should strictly follow this format:
Thought: {Provide your reasoning and explanation}
Code: {Output your executable Python code wrapped in ```python ``` marks}
...
def ck_plan(**kwargs):
    user_lines = []
    task_summary = kwargs["task"]
    recent_steps_summary = _summarize_recent_steps(kwargs["recent_steps_str"])
    state_summary = _summarize_state(kwargs["state"])

    user_lines.append(f"## Target Task\n{task_summary}\n\n")
    user_lines.append(f"## Recent Steps (latest)\n{recent_steps_summary}\n\n")
    user_lines.append(f"## Progress State Snapshot\n{state_summary}\n\n")
    user_lines.append(
        """## Output
...
Code: {Output your python dict of the updated progress state. Remember to wrap the code with "```python ```" marks and include only that single code block—do not append any additional ```python``` sections or action code.}
"""
    )
...
def ck_end(**kwargs):
    ...
    recent_steps_summary = _summarize_recent_steps(kwargs["recent_steps_str"])
    state_summary = _summarize_state(kwargs["state"])
    current_step_summary = _summarize_current_step(kwargs["current_step_str"])
...
def ck_aggr(**kwargs):
    ...
    task_summary = kwargs["task"]
    state_summary = _summarize_state(kwargs["state"])
    current_step_summary = _summarize_current_step(kwargs["current_step"])
